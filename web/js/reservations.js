...
async function loadReservationForm() {
  ...
  setDate();
  document
    .getElementById("txtBranch")
    .addEventListener("change", loadRoomsReservation);
  await loadSchedulesReservation();
  document
    .getElementById("txtRoom")
    .addEventListener("change", loadSchedulesReservation);
  document
...
async function loadRoomsReservation() {
  ...
  const status = 1;
  const url = `api/room/search?filter=${encodeURIComponent(
    filter
  )}&e=${status}`;
...
async function loadTreatmentDetails(treatment) {
  ...
  document
    .getElementById("btnSaveTreatmentDetails")
...
function saveReservation(e) {
  ...
  const clientId = document
    .querySelector(`#listClients option[value="${clientName}"`)
    .getAttribute("form-value");
  const date = document.getElementById("txtDate").value;
...
async function saveService(e) {
  ...
  const employeeId = document
    .querySelector(`#listEmployees option[value="${employeeName}"`)
    .getAttribute("form-value");
  ...
  const link = "api/service/insert/";
  let data = { s: JSON.stringify(servicio) };
  alertaGuardarNuevo(() => {
    $.ajax({
      type: "POST",
      dataType: "json",
